...
public class IRCodeGenerator implements IntermediateCodeGenerator {
	...
	*/
	...
	public static String saveIdentifier(String identifier,
		...
		if (!usedNames.contains(identifier)) {
			...
			irCode.add(QuadrupleFactory.declaration(identifier, type));
			return identifier;
		} else {
			...
			irCode.add(QuadrupleFactory.declaration(newName, type));
			return newName;
		}
	}
	...
	*/
	public static String loadIdentifier(String id) throws IntermediateCodeGeneratorException {
		...
		Stack<Map<String, String>> renameScopes = (Stack<Map<String, String>>) IRCodeGenerator.currentScopeRenames.clone();
		try {
			while (true) {
	...
	*/
	public static String createAndSaveTemporaryIdentifier(Type type) throws IntermediateCodeGeneratorException {
		...
		IRCodeGenerator.irCode.add(QuadrupleFactory.declaration(id, type));
		return id;
	}
