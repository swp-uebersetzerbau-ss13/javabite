...
public enum Mnemonic {
  ...
  JSR             ((byte) 0xA8),
  RET             ((byte) 0xA9),
  TABLESWITCH     ((byte) 0xAA), // missing args
  LOOKUPSWITCH    ((byte) 0xAB), // missing args
  IRETURN         ((byte) 0xAC),
  LRETURN         ((byte) 0xAD),
  FRETURN         ((byte) 0xAE),
  DRETURN         ((byte) 0xAF),
  ...
  //@formatter:on
	...
	/**
	 * Returns the mnemonic name of this bytecode operation. The mnemonic will be
	 * returned in all uppercase letters, because the enum-method name() is used
	 * to retrieve the mnemonic.
	 * 
	...
	/**
	 * Returns the bytecode operation byte as a string in hexadecimal format, eg.
	 * 01 or 5A. This is the string representation of the byte, and is created by
	 * calling Integer.toHexString(int). Note that the hexadecimal prefix 0x is
	 * missing. This prefix is intentionally omitted, so that this method can be
	 * used to visualize toe bytecode.
	 * 
